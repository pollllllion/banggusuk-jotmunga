-- ============================================================
-- 댓글·답글에 이미지/움짤·유튜브 붙이기
-- 2026-09-22
-- ============================================================
-- 무엇을:
--   1) discussion_comments."bodyHtml" — 서식 있는 댓글 본문(<img>·<div data-yt>).
...
--
-- 새 버킷은 만들지 않는다:
--   짤은 기존 'talk-media' 버킷(migration_talk_media)에 그대로 올린다. 읽기 공개·쓰기 익명이라는
--   조건이 글 첨부와 똑같고, 버킷을 늘리면 정책을 한 벌 더 관리해야 한다.
--   버킷의 서버 한도(20MB·이미지 계열)는 손대지 않는다 — 댓글은 클라이언트에서 5MB·1개로 더 조인다
--   (utils/talkMedia.ts 의 COMMENT_MAX_*). 서버 한도는 '최대 허용치', 클라이언트가 실제 관문이다.
--
