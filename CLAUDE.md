...
## "끝" — 점검 후 main 커밋·푸시 (= 배포)

사용자가 **"끝"** 한 단어로 말하면 아래를 **하나도 빼지 않고** 실행한다. 이번 세션에 내가 쓴 코드라도 생략하지 않는다.

1. **`npm run release-check`** (빌드 포함, `--no-build` 금지). 1~4단계를 돌리고 결과표를 찍는다.
   - ❌ 가 있으면 **푸시 금지** → 고치고 다시 실행
2. **⚠️ 항목은 전부 직접 판정한다.** 스크립트는 "어디를 볼지" 까지만 알려준다.
   - 기능 손실: 표에 나온 변경 큰 파일 순으로 **diff 를 실제로 읽는다**
   - SQL: 표에 나온 파일이 운영 DB 에 적용됐는지 확인(필요하면 RPC 로 부작용 없이 검증)
3. **사용자에게 결과표를 보고한다** — 스크립트 표 그대로 + ⚠️ 마다 무엇을 읽고 어떻게 판정했는지.
   전역 "간결하게" 규칙은 이 보고에 적용하지 않는다. 표를 요약하거나 단계를 빼지 말 것
4. `git add -A` → 커밋 → `git push origin main`
5. **배포 확인**: 라이브 `index.html` 의 asset 이름이 바뀌었는지 + 그 번들에 **이번 커밋에만 있는 ASCII 문자열**
   (함수명 말고 문자열 리터럴 — 이름은 압축된다)이 들어 있는지. 로컬·라이브 해시 비교는 영원히 안 맞으니 쓰지 말 것

### 아키텍처 불변식 (4단계가 검사하는 것)

1. **데이터 접근은 `dataService` 한 겹** — 페이지·컴포넌트가 `supabase` 를 직접 부르지 않는다. 읽기는 캐시에서 동기, 쓰기는 `store()` → write-through. (캐시 대상이 아닌 Storage 업로드 `src/utils/talkMedia.ts` 는 예외)
2. **시작 로드 캐시에 무거운 컬럼 금지** — 앱 시작 시 테이블을 통째로 긁는다. base64·긴 본문을 행에 담지 않는다(`castMembers`·`backdropUrl` 을 뺀 이유)
3. **RLS 우회 금지** — 유동닉의 쓰기·수정·삭제는 비번을 검증하는 `security definer` RPC 로만. 클라이언트에서 해시 비교 후 직접 update 하지 않는다
4. **프리렌더가 읽는 필드를 앱 전용 형식으로 바꾸지 않기** — `scripts/*.mjs` 가 DB 를 직접 읽어 SEO 를 만든다(`bodyHtml` 을 넣으면서 `body` 평문을 유지한 이유). SEO 메타는 앱과 `src/shared/*.mjs` 가 갈리지 않게
5. **마이그레이션은 SQL Editor 수동 적용** — 새 컬럼은 적용 전에도 앱이 죽지 않게 옵셔널로

검사 규칙을 바꾸려면 `scripts/release-check-lib.mjs` 와 그 테스트를 함께 고친다.
