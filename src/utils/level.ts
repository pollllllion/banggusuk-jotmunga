 ...
 * 진해지게 했다(LevelMark.tsx).
 *
 * ★ 2026-09-17 (같은 날 재조정) — 초반이 너무 빨랐다. 한 칸이 2~3 XP 라 시청 등록(20)+출석(20)만
 *   채워도 40 XP = Lv.14 였고, 글 1개 쓴 회원이 Lv.14, 3개 쓴 회원이 Lv.16 이었다.
 *   사다리 전체를 3배쯤 늘렸다. 등급 경계도 같이 옮겼다(25·90 → 80·310).
 *
 * LEVEL_MINS[n-1] = Lv.n 이 되는 최소 XP. 간격은 위로 갈수록 벌어진다:
 *   백수  한 칸 5~13 XP  — 무발화 상한(시청20+댓글30+출석20 = 70)을 다 채워도 Lv.10 에서 멈춘다.
 *                          **글을 안 쓰면 백수를 못 벗어난다.** 시청+출석만(40)이면 Lv.7
 *   한량  한 칸 14~32 XP — 80 부터. 무발화 상한에 추천 상한(40)까지 얹어도 110 = Lv.13
 *   여포  한 칸 40 → 320 XP — **일부러, 그리고 갈수록 더 어렵게.** 칸마다 폭이 1.25배 이상씩 불어난다
 *                          (40·50·65·85·110·145·190·250·320). 310 은 무발화·추천 상한을 다 채우고도
 *                          장문 50편이 더 필요하다. 만렙(1565)은 장문 360편쯤 — 하루 한 편씩 써도 1년이다.
...
export const LEVEL_MINS = [
  0, 5, 10, 16, 22, 29, 37, 46, 56, 67,                 // 백수 Lv.1~10
  80, 94, 110, 128, 148, 170, 194, 220, 248, 278,       // 한량 Lv.11~20
  310, 350, 400, 465, 550, 660, 805, 995, 1245, 1565,   // 여포 Lv.21~30
] as const
