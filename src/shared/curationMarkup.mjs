 ...
 *   빈 줄        문단 나눔 (예전부터 쓰던 규칙 그대로)
 *   ## 소제목     ### 도 같은 급으로 본다
 *   - 항목        목록 (* 도 됨)
 *   | a | b |     표 — 둘째 줄이 | --- | --- | 이면 첫 줄이 머리글. 챗GPT가 주는 표를 그대로 붙여 넣으면 된다
 *   **굵게**      문단·목록·표 칸 어디서나
 *
  ...
  for (const chunk of chunks(text)) {
    ...
    while (i < lines.length) {
      ...
      if (isTableLine(line)) {
        ...
        const run = []
        while (i < lines.length && !isTableLine(lines[i]) && !LIST_RE.test(lines[i]) && !HEAD_RE.test(lines[i])) {
          run.push(lines[i++])
        }
        const t = run.join('\n').trim()
