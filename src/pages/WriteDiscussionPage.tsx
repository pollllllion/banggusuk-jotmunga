...
/** 방구석토론방 글쓰기 — 팝업이 아닌 전체 페이지. 작품을 고르고 글을 쓴다.
...
export function WriteDiscussionPage() {
  ...
  const matches = useMemo(() => {
    ...
    if (!query) return []
    return DS.getContents().filter(c => c.title.toLowerCase().includes(query)).slice(0, 20)
  }, [q])
  ...
  const submit = async () => {
    ...
    try {
      let created
      if (isAccount && user) {
        created = DS.createDiscussion({ contentId: picked.id, authorId: user.id, title: head, body: text })
      } else {
        ...
        created = DS.createDiscussion({ contentId: picked.id, authorId: null, guestName: guestName.trim(), guestPwHash: hash, title: head, body: text })
      }
      toast('글을 올렸어요!')
      ...
      <div className="disc-write-page fade-in">
        ...
        {!picked ? (
          ...
          <>
            ...
            />
            <textarea
              className="form-input" style={{ marginTop: 8, minHeight: 240, resize: 'vertical' }}
