...
export function StarIcon({ size = 15, color = 'currentColor', filled = true }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? color : 'none'} stroke={color} strokeWidth="2">
...
export function ThumbUpIcon({ size = 15, filled = false }: IconProps) {
  return (
...
export function ThumbDownIcon({ size = 15, filled = false }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
...
export function BellIcon({ size = 20, filled = false }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
...
/* 톱니바퀴 — 2026-09-17 전에는 원 + 짧은 선 8개라 해(라이트 모드) 아이콘으로 읽혔다.
   바로 옆에 진짜 해/달(테마 전환)이 생기면서 모양을 톱니로 바꿨다 */
